...
namespace Ashbound
{
    ...
    public sealed class CampHub:MonoBehaviour
    {
        ...
        private bool settingsOpen;private bool resourceHudVisible=true;private string message="";private float panelAlpha;private float gainTimer;private ExpeditionResource lastGain;private int lastGainAmount;private int selectedStarterPlayer;
        ...
        {
            float x=742;U.Panel(new Rect(x,18,510,58));for(int i=0;i<Resources.Length;i++){var resource=Resources[i];float bx=x+12+i*122;float pulse=resourcePulse.GetValueOrDefault(resource);if(pulse>0)U.Box(new Rect(bx-3,24,117,44),new Color(1,.52f,.18f,.16f*pulse));GUI.Box(new Rect(bx,28,32,32),new GUIContent(ResourceIconLibrary.Icon(resource),LocalizationService.ResourceDescription(resource)));U.Label(bx+39,31,70,29,run.Progression.Profile.currencies.Get(resource).ToString(),U.Resource);}if(!string.IsNullOrEmpty(GUI.tooltip)){Vector2 mouse=Event.current.mousePosition;U.Panel(new Rect(Mathf.Clamp(mouse.x,760,980),85,260,72));U.Label(Mathf.Clamp(mouse.x,760,980)+10,92,240,58,GUI.tooltip,U.Tooltip);}if(gainTimer>0)U.Label(980,82,245,26,"+"+lastGainAmount+" "+LocalizationService.ResourceName(lastGain),U.Resource);
        }
        ...
        {
            U.Label(210,305,370,25,LocalizationService.Text("camp.preparation","ONE PREPARATION"),U.CardTitle);int i=0;foreach(var prep in run.Catalog.preparations){bool available=run.Progression.PreparationAvailable(prep);string label=(run.Progression.Profile.selectedPreparation==prep.id?"✓ ":"")+LocalizationService.PreparationName(prep);float x=210+(i%2)*205,y=333+(i/2)*44;GUI.enabled=available;if(U.Click(new Rect(x,y,195,U.PreparationButtonHeight),label))run.Progression.SelectPreparation(prep);GUI.enabled=true;i++;}
            U.Label(650,305,215,23,LocalizationService.Text("camp.party","LOCAL PARTY"),U.CardTitle);GUI.enabled=run.Lobby.Slots.Count<4&&run.Lobby.Slots.All(s=>s.InputKind!=InputKind.SecondKeyboard);if(U.Click(new Rect(870,300,82,28),"+ PLAYER"))run.Lobby.TryJoin(InputKind.SecondKeyboard,-2,"Shared keyboard");GUI.enabled=run.Lobby.Slots.Count>1;if(U.Click(new Rect(958,300,82,28),"REMOVE"))run.Lobby.RemoveLast();GUI.enabled=true;
            selectedStarterPlayer=Mathf.Clamp(selectedStarterPlayer,0,Mathf.Max(0,run.Lobby.Slots.Count-1));for(int p=0;p<run.Lobby.Slots.Count;p++){var slot=run.Lobby.Slots[p];if(U.Click(new Rect(650+p*98,333,92,27),slot.PlayerId+(p==selectedStarterPlayer?" *":"")))selectedStarterPlayer=p;}
            if(run.Lobby.Slots.Count>0){var slot=run.Lobby.Slots[selectedStarterPlayer];var selected=run.Catalog.FindWeapon(run.StarterWeapon(slot.PlayerId));U.Label(650,363,390,22,slot.PlayerId+" STARTER · "+selected.displayName+" · COMMON",U.Small);var families=(WeaponFamily[])Enum.GetValues(typeof(WeaponFamily));for(int w=0;w<families.Length;w++){float x=650+(w%4)*98,y=388+(w/4)*31;string label=families[w].ToString().Replace("DualBlades","Dual Blades");if(U.Click(new Rect(x,y,92,27),label+(run.StarterWeapon(slot.PlayerId)==families[w]?" *":"")))run.SetStarterWeapon(slot.PlayerId,families[w]);}}
            if(U.Click(new Rect(210,430,400,36),LocalizationService.Text("camp.launch","LAUNCH EXPEDITION")+" · "+run.Lobby.Slots.Count))LaunchExpedition();
        }
