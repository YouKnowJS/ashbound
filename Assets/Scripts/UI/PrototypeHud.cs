    ...
    public sealed class PrototypeHud : MonoBehaviour
    {
        ...
        private void Lobby()
        {
            ...
            if(facility.kind==HubFacilityKind.ExpeditionTable)
            {
                ...
                U.Label(70,342,700,25,LocalizationService.Text("camp.preparation","ONE PREPARATION"),U.CardTitle);int prep=0;foreach(var definition in run.Catalog.preparations){bool prepAvailable=run.Progression.PreparationAvailable(definition);GUI.enabled=prepAvailable;if(U.Click(new Rect(70+(prep%2)*350,375+(prep/2)*44,335,U.PreparationButtonHeight),(run.Progression.Profile.selectedPreparation==definition.id?"✓ ":"")+LocalizationService.PreparationName(definition)))run.Progression.SelectPreparation(definition);GUI.enabled=true;prep++;}
            }
