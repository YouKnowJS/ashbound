...
namespace Ashbound.Editor
{
    public sealed class PrototypeContentBuilder : IPreprocessBuildWithReport
    {
        ...
        public static void CreateContent()
        {
            ...
            var boss = Asset<BossDefinition>("Assets/ScriptableObjects/Bosses/CinderRegent.asset", x => { x.corruption=corruption;x.navigationRadius=1.2f;x.minimumObstacleClearance=.4f;x.preferredMovementZone=.76f;x.allowedArenaSections=Array.Empty<string>();x.stuckDetectionSeconds=1.5f;x.minimumStuckDisplacement=.12f;x.recoveryAttemptsBeforeReposition=3;x.preferredAttackDistance=4.5f;x.recoveryDuration=1.35f;x.phaseTwoRecovery=.82f;x.sweepRadius=5.2f;x.sweepDamage=28;x.chargeDistance=11;x.chargeSpeed=15;x.chargeDuration=.65f;x.chargeDamage=24;x.flameRingRadius=7.2f;x.flameRingWidth=1.35f;x.flameRingDamage=22;x.hazardDamage=13;x.hazardDuration=4.5f; });
        ...
        private static ExpeditionRegionDefinition CreatePrototypeRegion(EncounterDefinition[] encounters,CombatSpaceDefinition[] spaces,TreasureDefinition[] treasures,MerchantDefinition[] merchants,RestNodeDefinition[] rests,EventDefinition[] events,ChallengeDefinition[] challenges,BossRewardDefinition[] bossRewards,Vector3[] points)
        {
            ...
            ExpeditionRouteGraphDefinition Graph(string suffix,string title,VoteTieBehavior tie,int eventOffset)
            {
                ...
                ExpeditionNodeDefinition N(string key,string name,string description,ExpeditionNodeType type,NodeRiskRating risk,NodeRewardCategory reward,string[] next,EncounterDefinition encounter=null,CombatSpaceDefinition space=null)=>Asset<ExpeditionNodeDefinition>("Assets/ScriptableObjects/Routes/Nodes/"+Id(key)+".asset",x=>{bool combat=type==ExpeditionNodeType.NormalCombat||type==ExpeditionNodeType.HardCombat||type==ExpeditionNodeType.Elite||type==ExpeditionNodeType.Challenge||type==ExpeditionNodeType.Boss;x.id=Id(key);x.displayName=name;x.description=description;x.nodeType=type;x.risk=risk;x.rewardCategory=reward;x.rewardQuality=type==ExpeditionNodeType.NormalCombat?RewardQuality.Common:type==ExpeditionNodeType.HardCombat||type==ExpeditionNodeType.Challenge?RewardQuality.Rare:type==ExpeditionNodeType.Elite||type==ExpeditionNodeType.Boss?RewardQuality.Epic:RewardQuality.Advanced;x.outgoingConnections=next.Select(Id).ToArray();x.encounter=encounter;x.combatSpace=space;x.spawnPoints=points;x.resourceReward=new ResourceWallet();x.grantEquipment=combat;x.grantRelic=combat;x.treasure=null;x.merchant=null;x.rest=null;x.eventDefinition=null;x.challenge=null;x.finalBoss=null;x.bossReward=null;x.isTrueFinalBoss=false;x.telemetryTags=new[]{"region-1",suffix,key};});
                var start=N("start","Threshold Patrol","A readable opening combat anchors the route.",ExpeditionNodeType.NormalCombat,NodeRiskRating.Low,NodeRewardCategory.Resources,new[]{"hard","treasure"},Enc("frontline-pressure"),Space(1));start.resourceReward=new ResourceWallet{ash=10};
                ...
                var hard=N("hard","Broken Formation","A more demanding formation pays resources, relic, and equipment drafts.",ExpeditionNodeType.HardCombat,NodeRiskRating.High,NodeRewardCategory.Equipment,hardEdges,Enc(suffix=="b"?"ground-ambush":"collapse"),Space(1));hard.resourceReward=new ResourceWallet{ash=15,emberShards=2};hard.rewardQuality=RewardQuality.Rare;
                var treasure=N("treasure","Veiled Cache","The node identity is known; its weighted cache behavior is not.",ExpeditionNodeType.Treasure,NodeRiskRating.Moderate,NodeRewardCategory.Variable,treasureEdges,null,Space(0));treasure.treasure=treasures[0];
                var relic=N("relic","Ashbound Reliquary","A dedicated relic cadence choice without a preceding combat.",ExpeditionNodeType.Relic,NodeRiskRating.Low,NodeRewardCategory.Relic,relicEdges,null,Space(0));relic.grantRelic=true;
                var elite=N("elite","Warden Escort","An elite composition offers stronger relic and equipment drafts plus rare materials.",ExpeditionNodeType.Elite,NodeRiskRating.Severe,NodeRewardCategory.Equipment,eliteEdges,Enc("control-group"),Space(3));elite.resourceReward=new ResourceWallet{ash=20,emberShards=5};elite.rewardQuality=RewardQuality.Epic;
                var merchant=N("merchant","Ember Quartermaster","Spend expedition resources on finite stock.",ExpeditionNodeType.Merchant,NodeRiskRating.Low,NodeRewardCategory.Merchant,new[]{"rest","event"},null,Space(0));merchant.merchant=merchants[0];
                var rest=N("rest","Quiet Brazier","Choose one recovery or tempering action.",ExpeditionNodeType.Rest,NodeRiskRating.Low,NodeRewardCategory.Recovery,new[]{"challenge"},null,Space(0));rest.rest=rests[0];
                var eventNode=N("event",events[eventOffset%events.Length].displayName,"A two-to-three choice authored event.",ExpeditionNodeType.Event,NodeRiskRating.Moderate,NodeRewardCategory.Variable,new[]{"challenge"},null,Space(0));eventNode.eventDefinition=events[eventOffset%events.Length];
        ...
        private static ProgressionTuningDefinition CreateProgressionTuning()=>Asset<ProgressionTuningDefinition>("Assets/ScriptableObjects/Meta/progression-tuning.asset",x=>
        {
            ...
            x.rarity=new RewardRarityTuning{early=new RarityWeights{common=62,advanced=30,rare=8},mid=new RarityWeights{common=18,advanced=44,rare=31,epic=7},late=new RarityWeights{common=3,advanced=17,rare=56,epic=23,legendary=1},boss=new RarityWeights{advanced=4,rare=56,epic=37,legendary=3},regionRareStep=.08f,riskRareStep=.12f,eliteRareMultiplier=1.45f,qualityStepMultiplier=1.18f,metaRareMultiplier=1,preparationRareMultiplier=1.25f};
            x.threatBudget=new ThreatBudgetTuning{baseDensityMultiplier=1.2f,depthMultiplier=.35f,riskStep=.12f,eliteBonus=.28f,playerBonusPerExtraPlayer=.48f,reinforcementDelay=.8f,maximumEnemies=18};
        ...
        private static WeaponDefinition[] CreateWeapons(WeaponSkillDefinition[] skills)
        {
            ...
            result[1].passiveDescription="Focused thrusts reward exact spacing.";EditorUtility.SetDirty(result[1]);
            result[3].passiveDescription="Dash timing improves precision pressure.";EditorUtility.SetDirty(result[3]);
            return result;
        ...
        private static ItemDefinition Item(string id, string name, string description, BuildTag[] tags, StatModifiers modifiers,
            ...
            Asset<ItemDefinition>("Assets/ScriptableObjects/Items/" + id + ".asset", x =>
            {
                ...
                x.requiredItemId = requirement;bool capstone=new[]{"inferno-core","deep-winter","thunderhead","toxicity","entropy"}.Contains(id);x.rarity=capstone?Rarity.Epic:!string.IsNullOrEmpty(requirement)||id=="rupture"||id=="forked-heart"?Rarity.Rare:Rarity.Advanced;
            });
